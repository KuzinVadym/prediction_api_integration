...
export const repliesRoutes: IRouterBuilder = (getState: Function): Router => {
  ...
  router.put('/:id', async (req, res) => {
    const result = await repliesService.updateReply({id: req.params.id, update: req.body});
    res.json(result);
  });

  router.delete('/:id', async (req, res) => {
    const result = await repliesService.deleteReply({id: req.params.id, update: req.body});
    res.json(result);
  });
